...
void desktop_vec_add(struct desktop_vec *restrict vec, const char *restrict id,
...
void desktop_vec_add_file(struct desktop_vec *vec, const char *id,
                          const char *path) {
   ...
   */
  char *keywords =
      g_key_file_get_locale_string(file, group, "Keywords", NULL, NULL);
  if (keywords == NULL) {
    keywords = xmalloc(1);
    *keywords = '\0';
  }
  ...
  gsize length;
  gchar **list =
      g_key_file_get_string_list(file, group, "OnlyShowIn", &length, NULL);
  if (list) {
...
struct desktop_entry *desktop_vec_find_sorted(struct desktop_vec *restrict vec,
                                              const char *name) {
  /*
...
struct string_ref_vec desktop_vec_filter(const struct desktop_vec *restrict vec,
...
// }
//
// static void desktop_vec_add_file2(struct desktop_vec *desktop, const char
// *id, const char *path)
// {
...
// 		if (!strncmp(line, "[Desktop Entry]", strlen("[Desktop
// Entry]"))) { 			found = true; 			break;
// 		}
// 	}
// 	if (!found) {
